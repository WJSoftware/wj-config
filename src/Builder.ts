import type { ConfigurationValue, IBuilder, IDataSource, IEnvAwareBuilder, IEnvironment, IncludeEnvironment, Predicate, ProcessFetchResponse, Traits, UrlBuilderSectionWithCheck } from "wj-config";
...
interface IEnvironmentSource<TEnvironments extends string> {
    name?: string,
...
export default class Builder<T extends Record<string, any> = {}> implements IBuilder<T> {
    #controlData: BuilderImpl = new BuilderImpl();
    add<NewT extends Record<string, any>>(dataSource: IDataSource<NewT>) {
        this.#controlData.add(dataSource);
    ...
    when(predicate: () => boolean, dataSourceName?: string) {
        this.#controlData.when(predicate, dataSourceName);
        return this;
    }
    ...
    includeEnvironment<TEnvironments extends string, TEnvironmentKey extends string = "environment">(
        ...
        this.#controlData._lastCallWasDsAdd = false;
        ...
        return new EnvAwareBuilder<TEnvironments, Omit<T, TEnvironmentKey> & IncludeEnvironment<TEnvironments, TEnvironmentKey>>(_envSource, this.#controlData);
    }

    createUrlFunctions<TUrl extends keyof T>(wsPropertyNames: TUrl | TUrl[], routeValuesRegExp?: RegExp) {
        this.#controlData.createUrlFunctions(wsPropertyNames, routeValuesRegExp);
    ...
    async build(traceValueSources: boolean = false, enforcePerEnvironmentCoverage: boolean = true) {
        return this.#controlData.build(traceValueSources, p => p()) as unknown as T;
    }
};
